...
function Ataques() {
  ...
  function openAttack(id) {
    setPersonagem(prev => ({
      ...prev,
      attacks: prev.attacks.map(a => a.id === id ? { 
        ...a, 
        open: !a.open, 
      } : a)
    }));
    ...
    <div className="form-section">
      ...
      <div className="dynamic-list mt-3">
        {personagem.attacks.map(attack => (
          <div className="card attack-block mb-2 shadow-sm p-2 rounded" key={attack.id}>
            <div className="row g-2">
              <div className="col-md-4">
                <label className="form-label">Nome do Ataque</label>
                <input type="text" className="form-control fw-bold" value={attack.attackname} onChange={e => handleChange(attack.id, 'attackname', e.target.value)} />
              </div>
              ...
              <div className="col-md-1 d-flex align-items-center pt-3">
                ...
                <button className="btn btn-outline-secondary mr-2" onClick={() => openAttack(attack.id)}>
                  <i className={`fa-solid ${attack.open ? 'fa-caret-up' : 'fa-caret-down'}`}></i>
                </button>
              </div>
            </div>
            <div hidden={!attack.open}>
              <div className="row g-2">
                <div className="col-md-2">
              ...
              <div className="mt-3">
                <textarea className="form-control fst-italic" placeholder="Descrição" rows="3" value={attack.attackdesc} onChange={e => handleChange(attack.id, 'attackdesc', e.target.value)} />
              </div>
            </div>
          </div>
        ))}
